...
def __query_mutate_contains_op(match):
    ...
    return (
        f"({column_name}.notna() & {column_name}.str.contains(\'{value}\'))"
        if operator == Operator.CONTAINS
        else f"~({column_name}.notna() & {column_name}.str.contains(\'{value}\'))"
    )
...
def query_with_action_code(df, action_code, kwargs):
    ...
    for match in ACTION_CODE_PATTERN.finditer(action_code):
        ...
        if operator in [Operator.CONTAINS, Operator.NOT_CONTAINS]:
            ...
            operator in [Operator.EQUALS, Operator.NOT_EQUALS]
            and value == 'null'
        ):
